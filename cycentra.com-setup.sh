...
# ── Step 6: Marketplace token (.env) ─────────────────────────────────────────
step_header "MARKETPLACE TOKEN"

_ENV_FILE="${DEPLOY_DIR}/.env"

# On --update / --upgrade keep the existing token if already configured.
_EXISTING_TOKEN=""
if [[ -f "$_ENV_FILE" ]]; then
    _EXISTING_TOKEN=$(grep -E '^MARKETPLACE_CATALOG_TOKEN=' "$_ENV_FILE" 2>/dev/null \
        | head -1 | cut -d'=' -f2- | tr -d '"' || true)
fi

if [[ -n "$_EXISTING_TOKEN" ]]; then
    success "MARKETPLACE_CATALOG_TOKEN already set in ${_ENV_FILE} — keeping existing value"
else
    # Generate a fresh token automatically so the site works out of the box.
    # Admins must copy this value into /opt/cycentra/.env on their cycentra360
    # server so both sides share the same secret.
    _NEW_TOKEN=$(python3 -c "import secrets; print(secrets.token_hex(32))" 2>/dev/null \
        || openssl rand -hex 32)

    # Write .env (readable by root only)
    cat > "$_ENV_FILE" <<EOF
# CyCentra website — runtime environment
# Generated by cycentra.com-setup.sh on $(date -u +"%Y-%m-%d %H:%M UTC")

# ── Marketplace token ──────────────────────────────────────────────────────────
# This token gate protects /marketplace/catalog.json from unauthorised access.
# Copy this value into /opt/cycentra/.env on every cycentra360 server:
#   MARKETPLACE_CATALOG_TOKEN=${_NEW_TOKEN}
MARKETPLACE_CATALOG_TOKEN=${_NEW_TOKEN}
...
EOF
    ...
    echo -e "  ${BOLD}${YELLOW}  ║  ACTION REQUIRED — copy token to cycentra360 server         ║${NC}"
    echo -e "  ${BOLD}${YELLOW}  ╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  Add this line to ${BOLD}/opt/cycentra/.env${NC} on your cycentra360 server:"
    echo ""
    echo -e "  ${CYAN}  MARKETPLACE_CATALOG_TOKEN=${_NEW_TOKEN}${NC}"
    echo ""
    echo -e "  Then restart the backend:  ${DIM}sudo systemctl restart cycentra${NC}"
    echo ""
fi
